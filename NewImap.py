import binascii, errno, random, re, socket, subprocess, sys, time, calendar, socks
...
Literal = re.compile('.*{(?P<size>\\d+)}$', re.ASCII)
MapCRLF = re.compile('\\r\\n|\\r|\\n')
Response_code = re.compile('\\[(?P<type>[A-Z-]+)( (?P<data>.*))?\\]')
Untagged_response = re.compile('\\* (?P<type>[A-Z-]+)( (?P<data>.*))?')
Untagged_status = re.compile('\\* (?P<data>\\d+) (?P<type>[A-Z-]+)( (?P<data2>.*))?', re.ASCII)
_Literal = '.*{(?P<size>\\d+)}$'
_Untagged_status = '\\* (?P<data>\\d+) (?P<type>[A-Z-]+)( (?P<data2>.*))?'
...
class IMAP4:
    ...
    def __init__(self, host='', port=IMAP4_PORT, timeout=None, proxies=None, proxytype=None):
        self.debug = Debug
        self.state = 'LOGOUT'
        self.literal = None
    ...
    def _connect(self):
        ...
        self.tagre = re.compile('(?P<tag>' + self.tagpre + '\\d+) (?P<type>[A-Z]+) (?P<data>.*)', re.ASCII)
    ...
    def _create_socket(self, timeout, proxies=None, proxytype=None):
        ...
        try:
            ...
            if proxies is not None:
                ipaddr, Port = proxies.split(':')
                if proxytype == 'http' or proxytype == 'https':
                    sock.set_proxy(socks.HTTP, ipaddr, int(Port))
                else:
                    if proxytype == 'socks4':
                        sock.set_proxy(socks.SOCKS4, ipaddr, int(Port), False)
                    else:
                        if proxytype == 'socks5':
                            sock.set_proxy(socks.SOCKS5, ipaddr, int(Port))
                        else:
                            raise ValueError('Unknown Proxy Type!')
        except:
            raise socks.ProxyError
...
if HAVE_SSL:
    ...
    class IMAP4_SSL(IMAP4):
        ...
        def __init__(self, host='', port=IMAP4_SSL_PORT, keyfile=None, certfile=None, ssl_context=None, timeout=None, proxytype=None, proxies=None):
        ...
        def _create_socket(self, timeout):
            sock = IMAP4._create_socket(self, timeout, self.proxies, self.proxytype)
